    ...
    .then(client => {
        ...
        app.get('/booklist', (req, res) => {
            myCollection.find().sort({_id: -1}).toArray()
                .then(result => {
                    res.render('bookList', { myList: result, title: "Book Library | Book List" });
                })
                .catch(err => console.log(err));
        })
        ...
        app.get('/booklist/:id', (req, res) => {
            ...
            myCollection.findOne({ _id: ObjectId(`${id}`)})
                .then(result => {
                    res.render('details', { singleBook: result, title: `Book Detail  -  ${result.title}` })
                })
                .catch(err => console.log(err));
        })
        ...
        app.delete('/booklist/:id', (req, res) => {
            const id = req.params.id;
            myCollection.deleteOne({_id: ObjectId(`${id}`)})
                .then(result => {
                    res.json({ redirect: '/booklist'})
                })
                .catch(err => console.log(err));
        })
        ...
        app.get('/', (req, res) => {
            res.render('index', { title: "Book Libray | Home - Add New Book" });
        })
        

        app.get('/contact', (req, res) => {
            res.render('contact', { title: "Book Library | Contact Me" });
        })

        app.use((req, res) => {
            res.status(404).render('404', { title: 'Book Library | 404, Page Not Found'});
        })
